...
class SquareMatrix
{
    ...
    SquareMatrix(const SquareMatrix & Matrix); //Copy Constructor
    SquareMatrix(SquareMatrix && Matrix); //Move Constructor
    SquareMatrix &operator= (const SquareMatrix &Matrix); //Copy Assignment 
    SquareMatrix &operator= (SquareMatrix &&Matrix); //Move Assignment
    int size() const; //int function for to return for the size value 
    T& at(int row,int column); //int function for row and column
    void resize(int new_size); //resize function to create new size for matrix
    bool operator== (const SquareMatrix &Matrix_A) const; //function to set matrices equal to each other
    SquareMatrix operator+ (const SquareMatrix &Matrix_A) ;  //function to set add the matrices together

    private:
    T** array_memory;//we do a double pointer to allocate pointers for a 2d array
    int matrixsize_; //matrix size
};
